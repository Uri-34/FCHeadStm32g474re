    ...
    : public FCPoint2D 
{
    ...
    inline bool operator>(const FCPoint3D &point) { return (_z > point._z || _x > point._x || _y > point._y); }
    inline bool operator>=(const FCPoint3D &point) { return (_z >= point._z || _x >= point._x || _y >= point._y); }
    inline bool operator<(const FCPoint3D &point) { return (_z < point._z || _x < point._x || _y < point._y); }
    inline bool operator<=(const FCPoint3D &point) { return (_z <= point._z || _x <= point._x || _y <= point._y); }
};
...
struct FCRect
{
    ...
    FCRect(const FCPoint2D leftUp = {0, 0}, const FCPoint2D rightDown = {0, 0}) 
        : _leftUp{leftUp}, 
...
struct FCCube
{
    ...
    inline bool operator>(FCCube &cube) { return volume() > cube.volume(); }
    inline bool operator>=(FCCube &cube) { return volume() >= cube.volume(); }
    inline bool operator<(FCCube &cube) { return volume() < cube.volume(); }
    inline bool operator<=(FCCube &cube) { return volume() <= cube.volume(); }
...
public:    
    ...
    inline FCSize2D& size() const { return size(); }
    inline void setSize(const FCSize2D &size) { _size = size; }

    inline bool isBound(FCPoint2D &point)
    {
        return isBounded<int16_t>(_point._x, point._x, _point._x + _size.width()) 
               && isBounded<int16_t>(_point._y, point._y, _point._y + _size.height());
    }

    inline bool isBound(int16_t x, int16_t y)
    {
        return isBounded<int16_t>(_point._x, x, _point._x + _size.width()) 
               && isBounded<int16_t>(_point._y, y, _point._y + _size.height());
    }
    ...
    const FCArea2D intersection(const FCArea2D& area);
};
    ...
    : public FCPoint3D
{
    ...
    FCArea3D(const FCPoint3D &point = FCPoint3D(), const FCSize3D &size = FCSize3D())
        : FCPoint3D{point},
          _size{size}
    {}
    ...
    {
        return isBounded<int16_t>(_x, point._x, _x +_size.width())
               && isBounded<int16_t>(_y, point._y, _y +_size.height())
