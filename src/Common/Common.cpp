...
const Area2D Area2D::intersection(const Area2D& area)
{
    int16_t left1 = _point._x;
    int16_t top1  = _point._y;
    int16_t right1 = left1 + _size._width;
    int16_t bottom1 = top1 + _size._height;

    int16_t left2 = area._point._x;
    int16_t top2  = area._point._y;
    int16_t right2 = left2 + area._size._width;
    int16_t bottom2 = top2 + area._size._height;
    ...
    {
        return Area2D(0, 0, 0, 0); // пустая область
    }

    return Area2D(left, top, right - left, bottom - top);
}
